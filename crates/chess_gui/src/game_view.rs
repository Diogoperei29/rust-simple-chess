use chess_core::{Game, Square, Piece, PieceType};
use egui::{Ui, Rect, Pos2, Vec2, Color32, Sense, FontId, Align2};
...
impl ChessGameView {
    ...
    pub fn draw(
        ...
        ui.horizontal(|ui| {
            ...
            if let Some(promo_square) = game.get_promotion_square() {
                ...
                self.draw_promotion_ui(ui, board_rect, square_size, promo_square, game, piece_assets);
            }
        });
    }
    ...
    pub fn draw_move_history(&self, ui: &mut Ui, game: &Game) {
        ...
        scroll_area.show(ui, |ui| {
            ...
            for (pair_index, (move_number, white_move, black_move)) in move_pairs.into_iter().enumerate() {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            if let Some(white_text) = white_move {
                                ...
                                let mut text = egui::RichText::new(white_text)
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            if let Some(black_text) = black_move {
                                ...
                                let mut text = egui::RichText::new(black_text)
                                    .font(FontId::monospace(12.0));
                                if is_last {
                                    text = text.strong();
                                }
    ...
    fn draw_board(
        ...
        for rank in 0..8 {
            for file in 0..8 {
                ...
                let is_last_move = game.last_move()
                ...
                if is_valid_move {
                    ...
                    if has_piece {
                        ...
                        painter.circle_stroke(
                            center,
                            radius,
                            egui::Stroke::new(square_size * 0.08, Color32::from_rgba_premultiplied(0, 0, 0, 100)),
                        );
                    } else {
                        let radius = square_size * 0.15;
                        painter.circle_filled(center, radius, Color32::from_rgba_premultiplied(0, 0, 0, 100));
                    }
                }
                ...
                if file == 0 {
                    painter.text(
                        ...
                        if is_light_square { Color32::from_rgb(118, 150, 86) } else { Color32::from_rgb(238, 238, 210) },
                    );
                }
            ...
            if response.dragged() {
                if let Some(piece) = game.get_piece(selected) {
                    if let Some(pos) = response.interact_pointer_pos() {
                        let drag_rect = Rect::from_center_size(pos, Vec2::new(square_size, square_size));
    ...
    fn draw_piece(&self, painter: &egui::Painter, rect: Rect, piece: Piece, piece_assets: &Option<PieceAssets>) {
    ...
    fn draw_promotion_ui(
        ...
        for (i, &piece_type) in promotion_options.iter().enumerate() {
            ...
            if response.hovered() {
                ...
                ui.painter().rect_filled(hover_rect, 0.0, Color32::from_rgb(255, 255, 0));
                ui.painter().rect_filled(piece_rect, 0.0, bg_color);
                self.draw_piece(ui.painter(), piece_rect, piece, piece_assets);
            }
        }
